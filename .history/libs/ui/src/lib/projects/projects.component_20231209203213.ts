...
export class ProjectsComponent implements OnInit {
  ...
  onDrop(event: CdkDragDrop<any[]>) {
    ...
    // Alternatively, for batch processing multiple update requests
    const batchedRequests = [
      ...
      // Add more batched requests if needed for additional project updates
    ];

    // Call the batch update method
    this.projectsService.updateOrderBetweenProjectsBatch(batchedRequests)
      .subscribe(() => {
        // Success message or further operations after successful batch reordering
      }, (error) => {
        // Handle error in case the batch reordering fails
      });
  }
