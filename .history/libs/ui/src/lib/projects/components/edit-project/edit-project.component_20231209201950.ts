  ...
  checkExistanceOfOrder(): void {
    if (this.projectForm.valid) {
      ...
      this._editProjectService.getAllProjects().subscribe((projects) => {
        const filteredProjects = projects.filter(project => project.id !== updatedData.id);
  
        if (filteredProjects.length === 0) {
          // If no other projects exist, directly update the current project
          this.executeUpdate(updatedData);
          return;
        }
  
        const orderExists = filteredProjects.some((project) => project.order === updatedData.order);
