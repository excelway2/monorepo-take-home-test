    ...
    if (this.projectForm.valid) {
      ...
      this._editProjectService.getAllProjects().subscribe((projects) => {
        ...
        if (orderExists) {
          ...
          // For instance, you can use a confirmation dialog (e.g., Swal.fire)
          Swal.fire({
            title: 'Order Already Exists',
            text: `The order you want to set already exists in ${orderExists.title}`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
              ...
              // Handle cancellation or perform other actions
            }
          });
        } else {
          this.executeUpdate(updatedData);
