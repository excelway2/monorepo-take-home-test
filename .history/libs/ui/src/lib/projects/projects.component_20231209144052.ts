...
export class ProjectsComponent implements OnInit {
  ...
  onDrop(event: CdkDragDrop<Project[]>): void {
    ...
    this.projectsService.getAllProjects().subscribe((projects) => {
      ...
      const reorderedProjectIds = projects.map((project) => project.id);
      const updatedData = { projectIds: reorderedProjectIds };

      // Update the backend with the new order
      this._editProjectService.updateProject(reorderedProjectIds, updatedData).subscribe(() => {
