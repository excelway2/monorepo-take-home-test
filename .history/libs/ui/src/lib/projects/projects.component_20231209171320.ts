...
export class ProjectsComponent implements OnInit {
  ...
  onDrop(event: CdkDragDrop<any[]>) {
    // Reordering logic
    const movedItem = this.projects[event.previousIndex];
    this.projects.splice(event.previousIndex, 1);
    this.projects.splice(event.currentIndex, 0, movedItem);

    // Update the order field in each project object
    this.projects.forEach((project, index) => {
      project.order = index + 1; // Update the order based on the new index
    });

    // Update the backend with the new order information
    this.projectService.updateProjectsOrder(this.projects).subscribe(() => {
      // Optional: Handle success or error response from backend
    });
  }
}