...
export class ProjectsComponent implements OnInit {
  ...
  onDrop(event: CdkDragDrop<Project[]>): void {
    // Rearranging projects logic locally
    moveItemInArray(this.projects, event.previousIndex, event.currentIndex);
  
    // Gather updated order information for all projects
    const updatedProjectsOrder = this.projects.map((project, index) => ({
      projectId: project.id,
      order: index // Assuming the backend uses 0-based indexing
    }));
  
    // Send updated order to the backend in a single request
    this.projectsService.updateProjectsOrder(updatedProjectsOrder)
      .pipe(
        catchError(error => {
