import { Controller, Get, Post, Put, Delete, Param, Body, NotFoundException, BadRequestException, InternalServerErrorException, UseFilters } from '@nestjs/common';
import { ProjectsService } from './projects.service';
import { Prisma, Project } from '@prisma/client';
import { ProjectsExceptions} from './projects.exceptions'; // Import the custom exception filter


@Controller('projects')
@UseFilters(ProjectsExceptions)
export class ProjectsController {
  ...
  async update(@Param('id') id: string, @Body() project: Partial<Project>): Promise<Project> {
    try {
      ...
      const updatedProject = await this.projectsService.update(id, project);
      if (!updatedProject) {
        // Throw a not found exception if project is not found
        throw new NotFoundException(`Project with id = ${id} not found.`);
      }
