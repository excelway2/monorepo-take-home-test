...
async updateOrderBetween(
  ...
  try {
    ...
    // Retrieve existing projects with order numbers
    const allProjects = await this.projectsService.getAllProjects();
    const orders = allProjects.map(project => project.order);

    if (orders.includes(order1) || orders.includes(order2)) {
      // If the new order already exists, adjust other projects' orders

      const highestOrder = Math.max(...orders);
      const newOrder = Math.max(order1, order2) + 1;

      for (const project of allProjects) {
        if (project.order >= newOrder && project.order <= highestOrder) {
          // Increment the order of projects that fall between the newOrder and highestOrder
          await this.projectsService.updateOrder(project.id, project.order + 1);
        }
      }
    }

    // Update the orders of the swapping projects
